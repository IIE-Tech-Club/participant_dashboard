            ...
            <div className="flex items-center gap-4 md:gap-8 shrink-0">
              <div>
                ...
                <div className="flex items-center gap-3">
                  <span className="font-mono-cc text-lg text-[#00f5ff]">{completedCount}/{totalPhases}</span>
                  <div className="w-24 h-1.5 bg-[rgba(255,255,255,0.1)] overflow-hidden">
                    <div className="h-full bg-[#00f5ff] transition-all duration-500 shadow-[0_0_8px_#00f5ff]" style={{ width: `${pct}%` }} />
                  </div>
                </div>
              </div>
              ...
              {hackathon.prize && (
                <div className="hidden md:block">
                  ...
                  <span className="font-mono-cc text-lg text-[#f59e0b] font-bold">{hackathon.prize}</span>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-5 py-4 overflow-x-auto scrollbar-hide">
          <div className="flex items-center min-w-max">
            {hackathon.phases.map((phase, idx) => {
              const unlocked = isPhaseUnlocked(hackathon.phases, progress.responses, phase.id);
              ...
              return (
                <div key={phase.id} className="flex items-center">
                  <button
                    ...
                    className="flex flex-col items-center gap-2 group outline-none relative"
                  >
                    {/* Active Glow */}
                    {isActive && (
                  ...
                  {idx < hackathon.phases.length - 1 && (
                    <div className="w-12 md:w-20 lg:w-32 px-2 -mt-5">
            ...
            {activePhaseObj ? (
              ...
              <div className="glass-card p-12 text-center border border-[rgba(0,245,255,0.2)]">
                ...
                <p className="font-mono-cc text-xs text-[rgba(224,247,255,0.4)] max-w-md mx-auto">
                  Select an unlocked phase from the mission tracker above to establish a link and begin data transmission.
                </p>
              </div>
            )}
          </div>
          
          {/* Right Column: Mission Command */}
          {hackathon.organizers && hackathon.organizers.length > 0 && (
            <div className="w-full lg:w-72 shrink-0 lg:sticky lg:top-[160px] animate-fade-up" style={{ animationDelay: '0.1s' }}>
              <div className="glass-card border border-[rgba(0,245,255,0.1)] bg-[rgba(2,6,23,0.4)] p-5">
                ...
                <div className="flex flex-col gap-4">
                  {hackathon.organizers.map((org, i) => (
                    <div key={i} className="relative group bg-[rgba(2,6,23,0.5)]">
                      ...
                      <div className="relative p-4 flex flex-col gap-4">
                        ...
                        <div className="flex items-center gap-3">
                          <div className="relative w-12 h-12 shrink-0 overflow-hidden bg-[rgba(0,245,255,0.05)] border border-[rgba(0,245,255,0.3)] group-hover:border-[#00f5ff] shadow-[0_0_10px_rgba(0,245,255,0.1)] group-hover:shadow-[0_0_15px_rgba(0,245,255,0.3)] transition-all duration-300">
                            {org.avatar ? (
                              <img src={org.avatar} alt={org.name} className="w-full h-full object-cover scale-100 group-hover:scale-110 transition-transform duration-500" />
                            ) : (
                        ...
                        <div className="flex gap-2">
                          {org.email && (
                            <a href={`mailto:${org.email}`} className="flex-1 flex items-center justify-center gap-2 py-2 px-2 bg-[rgba(255,255,255,0.03)] border border-[rgba(255,255,255,0.05)] hover:bg-[rgba(0,245,255,0.1)] hover:border-[#00f5ff] hover:text-[#00f5ff] hover:shadow-[0_0_15px_rgba(0,245,255,0.2)] text-[rgba(224,247,255,0.5)] transition-all duration-300">
                              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
                              <span className="font-orbitron font-bold text-[9px] uppercase tracking-[0.15em]">COMMS</span>
                            </a>
                          )}
                          {org.phone && (
                            <a href={`tel:${org.phone}`} className="flex-1 flex items-center justify-center gap-2 py-2 px-2 bg-[rgba(255,255,255,0.03)] border border-[rgba(255,255,255,0.05)] hover:bg-[rgba(0,245,255,0.1)] hover:border-[#00f5ff] hover:text-[#00f5ff] hover:shadow-[0_0_15px_rgba(0,245,255,0.2)] text-[rgba(224,247,255,0.5)] transition-all duration-300">
                              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg>
                              <span className="font-orbitron font-bold text-[9px] uppercase tracking-[0.15em]">LINK</span>
                            </a>
                          )}
                        </div>
                      </div>
